...
@router.get("/review/queue", response_model=list[ReviewQueueItem], dependencies=[Depends(require_permission("read"))])
def list_review_queue(task_id: UUID | None = None, db: Session = Depends(get_db)):
    return review_service.list_review_queue(db, task_id)


@router.get("/review/comments", response_model=list[ReviewCommentRead], dependencies=[Depends(require_permission("read"))])
...
@router.post("/review/comments", response_model=ReviewCommentRead, dependencies=[Depends(require_permission("review:write"))])
...
@router.patch("/fields/{field_id}", response_model=ExtractedFieldRead, dependencies=[Depends(require_permission("review:write"))])
...
@router.post("/audit-results/{result_id}/confirm", response_model=AuditResultRead, dependencies=[Depends(require_permission("review:write"))])
...
@router.post("/audit-results/{result_id}/dismiss", response_model=AuditResultRead, dependencies=[Depends(require_permission("review:write"))])
...
@router.post("/audit-results/{result_id}/rerun", response_model=list[AuditResultRead], dependencies=[Depends(require_permission("review:write"))])
