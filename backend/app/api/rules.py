...
@router.get("", response_model=list[AuditRuleRead], dependencies=[Depends(require_permission("read"))])
def list_rules(db: Session = Depends(get_db)):
    return rule_engine_service.list_rules(db)


@router.post("", response_model=AuditRuleRead, dependencies=[Depends(require_permission("rule:manage"))])
...
@router.patch("/{rule_id}", response_model=AuditRuleRead, dependencies=[Depends(require_permission("rule:manage"))])
...
@router.post("/{rule_id}/evaluate", response_model=list[AuditRuleEvaluateResult], dependencies=[Depends(require_permission("rule:manage"))])
