...
def _explicit_groups(
    ...
    for document in documents:
        doc_fields = fields.get(document.id, {})
        for field_name in ("payment_purpose", "summary"):
...
def _fallback_groups(
    ...
    for document in documents:
        if document.id in grouped_doc_ids:
            continue
        doc_fields = fields.get(document.id, {})
        supplier = _first_field(doc_fields, ("supplier_name", "seller_name", "payee_name"))
        amount = _first_field(
            doc_fields,
        ...
        document_date = _first_field(
            doc_fields,
            ("signing_date", "invoice_date", "payment_date", "receipt_date", "voucher_date", "request_date"),
        )
