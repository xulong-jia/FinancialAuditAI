...
SROIE-style public OCR labels are evaluated when present: every non-empty `key_information` value must appear in OCR `raw_text` or normalized `raw_text`, and `box_line_count` contributes a capped OCR-block lower bound. Metrics include `key_information_accuracy`, `box_line_count_coverage`, and `public_dataset_label_accuracy`.
