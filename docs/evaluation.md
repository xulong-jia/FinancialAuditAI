...
SRD and `1_Images`/Zenodo public image-only OCR robustness manifests can be run under `local_storage/external_acceptance/production_dataset/ocr`. The current local runs used five public images each, `ENVIRONMENT=test TESTING=true`, and the local `pymupdf-local` OCR path. Both completed with `failed_cases=[]`, `ocr_sample_pass_rate=1.0`, `page_count_accuracy=1.0`, `blocked_external_dependency_count=0`, `source_type=public_dataset`, `dataset_kind=non_production_manual_acceptance`, and `evaluation_status=non_production_manual_acceptance`. These manifests intentionally do not declare OCR text, bbox, table, confidence, or field labels because the archived datasets did not provide usable ground truth. Treat them only as public image ingestion/rendering robustness checks, not OCR quality, extraction quality, Provider, or production evidence.
