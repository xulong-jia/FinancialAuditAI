...
| manual acceptance regression dataset | 已支持 manifest + `regression.json`；当前样本聚合 OCR / classification / extraction / rule / RAG / Agent / E2E manual datasets，并禁止 regression 递归 | 只覆盖 manual acceptance 聚合，不代表生产级真实/脱敏大数据回归评测 |
