...
| Evaluation manual regression dataset | 已支持 `evals/datasets/<dataset>/dataset_manifest.json` 加载 `regression.json`；runner 聚合 `ocr`、`classification`、`extraction`、`rule`、`rag`、`agent`、`end_to_end` 七类 manual dataset evaluation，统计 per-eval sample_count、failed_cases_count、pass_rate、dataset-driven flag、production flag，并按 `all_required_eval_types_pass`、`max_failed_cases`、`required_dataset_driven`、`required_non_production_flag`、`required_eval_type_count` 断言；禁止 regression 递归调用 | `backend/app/services/evaluation_service.py`, `docs/evaluation.md`, `evals/datasets/manual_acceptance/regression.json` | `backend/tests/test_quality_api.py::test_manual_acceptance_regression_manifest_aggregates_dataset_results`, `backend/tests/test_quality_api.py::test_manual_acceptance_regression_manifest_blocks_recursive_eval_type` |
| Evaluation manual regression 真实运行 | 已成功运行 `manual_acceptance` regression dataset-driven evaluation；`sample_count=1`、`failed_cases=[]`、`regression_sample_pass_rate=1.0`、`required_eval_type_count=7`、`executed_eval_type_count=7`、`total_failed_cases=0`、`all_required_eval_types_pass=true`、`dataset_driven_coverage=1.0`、`non_production_flag_accuracy=1.0`；7 类前置 dataset 均 pass：OCR 1、classification 6、extraction 1、rule 2、RAG 2、Agent 2、E2E 1，均 `failed_cases_count=0`、`pass_rate=1.0` | 无 secret；该 dataset 为 synthetic/public non-production manual acceptance 聚合，不是生产级完整 Evaluation，也不是完整真实脱敏大数据集 | 手工运行：`eval_type=regression`, `dataset_path=evals/datasets/manual_acceptance/dataset_manifest.json` |
...
| 编号 | 模块 | 缺口 |
| --- | --- | --- |
| M-01 | Evaluation | 数据集驱动入口已存在；manual acceptance OCR、classification、extraction、rule、RAG、Agent、E2E、regression 8 类 runner 均已支持并已有 dataset-driven manual acceptance 结果；regression runner 聚合前述七类且 7 类前置 dataset 均 pass。classification/extraction/rule 完整 DB workflow、RAG persistent vector-store/workpaper scope、Agent agent_runs/agent_steps DB workflow、E2E 完整 DB/API workflow、真实/脱敏规模化 regression 仍依赖外部标注集和真实 Provider 配置。 |
...
| manual acceptance E2E dataset | 已支持 manifest + `e2e.json`；当前样本为 synthetic procurement walkthrough contract，模拟上传、OCR、分类、抽取、归集、规则、报告和 evidence index flag，不创建真实 DB/API workflow 或 report 文件 | 只覆盖 E2E contract 样本，不代表完整 task/document/OCR/classification/extraction/rule/report DB/API workflow 或生产级 Evaluation |
| manual acceptance regression dataset | 已支持 manifest + `regression.json` 并已跑通；当前样本聚合 OCR / classification / extraction / rule / RAG / Agent / E2E manual datasets，七类前置均 pass，并禁止 regression 递归 | 只覆盖 synthetic/public manual acceptance 聚合，不代表生产级真实/脱敏大数据回归评测 |
