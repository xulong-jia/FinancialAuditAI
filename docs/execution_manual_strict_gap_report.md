...
| `cd backend && ./.venv/bin/python -m pytest -q` | PASS, 155 passed, 5 warnings |
...
| H-02 | RAG | 四库、workpaper scope、citation、no-answer 存在；默认 embedding/rerank/answer 仍是 deterministic/local fallback，真实 embedding 维度与 pgvector 索引兼容性仍未完成验收。 |
| H-03 | Agent Workflow | 状态机、步骤、重试、Review 路由存在；与执行手册状态口径、Agent 工具职责和 Bad Case 工具闭环仍不完全一致。 |
...
1. RAG embedding provider 维度/索引兼容与真实 provider 验收。
2. Agent Workflow 状态、工具职责和 Bad Case 闭环继续对齐执行手册。
