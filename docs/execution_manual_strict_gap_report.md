...
| Evaluation full DB workflow dataset | 已新增 `full_db_workflow` eval type；runner 创建 runtime PDF、真实调用 task/OCR/classification/extraction/linkage/rule/report services，并验证 `audit_tasks`、`documents`、`document_pages`、`extracted_fields`、`document_relations`、`audit_results`、`reports`、`control_table_rows`、`evidence_refs` | `backend/app/services/evaluation_service.py`, `evals/datasets/manual_acceptance/full_db_workflow.json`, `docs/evaluation.md` | `backend/tests/test_quality_api.py::test_full_db_workflow_manifest_creates_persisted_artifacts` |
...
| 编号 | 模块 | 缺口 |
| --- | --- | --- |
| M-01 | Evaluation | 数据集驱动入口已存在；manual acceptance OCR、classification、extraction、rule、RAG、Agent、E2E、full DB workflow、regression runner 均已支持。full DB workflow 已验证核心 DB/service artifacts；RAG persistent vector-store/workpaper scope、Agent agent_runs/agent_steps DB workflow、真实/脱敏规模化 regression 仍依赖后续阶段或外部标注集和真实 Provider 配置。 |
...
| manual acceptance regression dataset | 已支持 manifest + `regression.json` 并聚合 OCR / classification / extraction / rule / RAG / Agent / E2E / full DB workflow manual datasets，并禁止 regression 递归 | 只覆盖 synthetic/public manual acceptance 聚合，不代表生产级真实/脱敏大数据回归评测 |
...
- 2026-07-06 本地 OpenAI-compatible readiness 已通过真实验证：普通 readiness 显示 LLM / embedding / RAG answer / RAG rerank 为 configured；`RUN_PROVIDER_INTEGRATION=1` 显示 LLM / embedding / RAG answer / RAG rerank 为 ready。`.env` 未提交，API key 未记录。普通 pytest 仍隔离真实 Provider；当前结果为 175 passed / 5 warnings。
...
- Evaluation Center 已开始支持 manual acceptance OCR dataset、classification text-sample dataset、extraction text-sample dataset、rule deterministic dataset、RAG inline-document dataset、Agent workflow contract dataset、E2E workflow contract dataset 和 regression aggregation dataset，路径为 `evals/datasets/manual_acceptance/dataset_manifest.json`、`ocr.json`、`classification.json`、`extraction.json`、`rule.json`、`rag.json`、`agent.json`、`e2e.json`、`regression.json`。当前 8 类 manual dataset runner 均已具备并已跑通；RAG persistent vector-store/workpaper scope、Agent agent_runs/agent_steps DB workflow、E2E 完整 DB/API workflow、真实/脱敏规模化 regression 仍待同等 dataset 化，不能据此声称 Evaluation Center 完全满足执行手册。
...
- Manual E2E dataset-driven evaluation 已跑通，但它是 synthetic single-sample non-production manual acceptance；本 runner 不调用完整真实 DB/API workflow。它不能作为生产级完整 Evaluation 或完整 task/document/OCR/classification/extraction/rule/report workflow 结论。
- Manual regression dataset-driven evaluation 已跑通，但它是 synthetic/public single-sample non-production manual acceptance aggregation；7 类前置 dataset 均 pass，不能作为生产级完整 Evaluation、真实/脱敏规模化回归评测或完整 DB/API workflow 结论。
