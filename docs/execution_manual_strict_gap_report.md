...
| H-02 | OCR Provider | OCR 已留痕且保留 confidence warning；当前主要依赖 PyMuPDF text/OCR，缺少可配置且能返回真实 block confidence 的 OCR Provider 闭环。 |
| H-03 | RAG | 四库、workpaper scope、citation、no-answer 存在；默认 embedding/rerank/answer 仍是 deterministic/local fallback，真实 embedding 维度与 pgvector 索引兼容性仍未完成验收。 |
| H-04 | Agent Workflow | 状态机、步骤、重试、Review 路由存在；与执行手册状态口径、Agent 工具职责和 Bad Case 工具闭环仍不完全一致。 |
...
| PyMuPDF OCR confidence unavailable | 仍存在 warning，不伪造 confidence | 是 |
| deterministic/local RAG embedding/rerank/answer | 仍存在，已留痕为 fallback | 是 |
...
1. OCR Provider 配置路径和真实 confidence 返回闭环。
2. RAG embedding provider 维度/索引兼容与真实 provider 验收。
3. Agent Workflow 状态、工具职责和 Bad Case 闭环继续对齐执行手册。
