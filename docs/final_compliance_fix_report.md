...
Manual extraction dataset support uses synthetic text samples and records `is_production_evaluation=false`. It checks fields, normalized values, item lines, and source traceability without calling a real LLM provider, but it is not a production-scale extraction evaluation and does not exercise the full uploaded-document DB workflow.
